...
try:
    ...
    class _EmpyricalFallback:
        @staticmethod
        def annual_return(rets: pd.DataFrame | pd.Series, annualization: int = 252):
            compounded = (1.0 + rets).prod()
            periods = rets.shape[0]
            if periods == 0:
                return np.nan
            return compounded ** (annualization / periods) - 1.0

        @staticmethod
        def cum_returns(rets: pd.DataFrame | pd.Series):
            return (1.0 + rets.fillna(0.0)).cumprod() - 1.0

    ep = _EmpyricalFallback()
...
class Factor_Calculator(BaseEstimator, TransformerMixin):
    ...
    def long_short_pct(
        ...
        step_c: pd.DataFrame = step_b.apply(lambda x: _ema(x, window1))
        step_d: pd.DataFrame = step_b.apply(lambda x: _ema(x, window2))
    ...
    def price_divergence(
        ...
        # 一阶量价背离
        rank_vol: pd.DataFrame = self.vol_df.pct_change(fill_method=None).apply(
            lambda x: x.dropna().rolling(window).rank()
        )
